...
def couldBeFeedData(data):
    ...
    if data_lower.count("<html"):
        return 0
    stripped = data.strip()
    if stripped.startswith("[") and '"rendered"' in stripped[:2000] and '"link"' in stripped[:2000]:
        return 1
    return data_lower.count("<rss") + data_lower.count("<rdf") + data_lower.count("<feed")
