NEWSBLUR.Views.FeedList = Backbone.View.extend({
    ...
    toggle_filter_feeds: function () {
        ...
        if (show_briefing && NEWSBLUR.assets.briefing_section_feeds.length) {
        ...
        // feed_list_view.js: Show Briefing Admin sidebar entry for staff users only
        if (NEWSBLUR.Globals.is_staff) {
            $('.NB-feeds-header-river-briefing-admin-container').css({
                'display': 'block',
                'opacity': 0
            }).animate({ 'opacity': 1 }, { 'duration': 700 });
        } else {
    ...
    make_briefing_sections: function (options) {
        ...
        if (NEWSBLUR.assets.briefing_section_feeds.length && NEWSBLUR.assets.preference('briefing_enabled')) {
            $('.NB-briefing-sections-folder').css({
                'display': 'block',
                'opacity': 0
            }).animate({ 'opacity': 1 }, { 'duration': options.update ? 0 : 700 });
